...
void compare_host_1D2_arrays(float2* h_float2_array, __nv_bfloat162* h_bfloat162_array, size_t data_length_bytes){
  ...
  int arr_length = (int) true_arr_length;
  printf("\n true_arr_length: %lf\n", true_arr_length);
  printf("array_index, bf16.x, bf16.y, fp32.x, fp32.y\n");
  for (int i = 0; i<arr_length; i++){
    if (!(((float)h_bfloat162_array[(int)i].x == 0.0) && ((float)h_bfloat162_array[(int)i].y == 0.0) && ((float)h_float2_array[(int)i].x == 0.0) && ((float)h_float2_array[(int)i].y == 0.0))) {
      //printf("nonzero element at index: %d, bf16.x: %.25f, bf16.y: %.25f, fp32.x: %.25f, fp32.y: %.25f\n",i,(float)h_bfloat162_array[(int)i].x,(float)h_bfloat162_array[(int)i].y,h_float2_array[(int)i].x,h_float2_array[(int)i].y);
      printf("%.25f, %.25f, %.25f, %.25f\n",(float)h_bfloat162_array[(int)i].x,(float)h_bfloat162_array[(int)i].y,h_float2_array[(int)i].x,h_float2_array[(int)i].y);
...
void print_1D_bfloat162_array(__nv_bfloat162* d_bfloat162_array, size_t data_length_bytes){
  ...
  void fdas_free_gpu_arrays_float(fdas_gpuarrays_float *arrays,  cmd_args *cmdargs)
  {
    ...
    if(e != cudaSuccess) {
      LOG(log_level::error, "Could not cudaFree in aa_fdas_host.cu float 1(" + std::string(cudaGetErrorString(e)) + ")");
    }
    
    e = cudaFree(arrays->d_fft_signal);

    if(e != cudaSuccess) {
      LOG(log_level::error, "Could not cudaFree in aa_fdas_host.cu float 2(" + std::string(cudaGetErrorString(e)) + ")");
    }
    
    e = cudaFree(arrays->d_ext_data);

    if(e != cudaSuccess) {
      LOG(log_level::error, "Could not cudaFree in aa_fdas_host.cu float 3(" + std::string(cudaGetErrorString(e)) + ")");
    }
    
    e = cudaFree(arrays->d_ffdot_pwr);

    if(e != cudaSuccess) {
      LOG(log_level::error, "Could not cudaFree in aa_fdas_host.cu float 4(" + std::string(cudaGetErrorString(e)) + ")");
    }
    
    e = cudaFree(arrays->d_kernel);

    if(e != cudaSuccess) {
      LOG(log_level::error, "Could not cudaFree in aa_fdas_host.cu float 5(" + std::string(cudaGetErrorString(e)) + ")");
    }
    ...
    if(cmdargs->basic) {
      ...
      if(e != cudaSuccess) {
  LOG(log_level::error, "Could not cudaFree in aa_fdas_host.cu float 6(" + std::string(cudaGetErrorString(e)) + ")");
      } 
    }
    ...
    if(cmdargs->kfft && cmdargs->inbin) {
      ...
      if(e != cudaSuccess) {
  LOG(log_level::error, "Could not cudaFree in aa_fdas_host.cu float 7(" + std::string(cudaGetErrorString(e)) + ")");
      } 
    }
  
    // Added by KA
    e = cudaFree(arrays->d_fdas_peak_list);
    if(e != cudaSuccess) {
      LOG(log_level::error, "Could not cudaFree in aa_fdas_host.cu float 7(" + std::string(cudaGetErrorString(e)) + ")");
    }
  ...
  void fdas_create_bfloat_acc_kernels(__nv_bfloat162* d_kernel, float2* temp_kernel_pointer, cmd_args *cmdargs) {
    ...
    long long int NUM_KERNEL_BYTES = KERNLEN*sizeof(float2)* NKERN;
    call_kernel_cast_float2_to_bfloat162(d_kernel, temp_kernel_pointer, NUM_KERNEL_BYTES);
...
//    long long int TOTAL_KERN_LEN = KERNLEN * NKERN;
//    for (long long int i_in_kernel = 0; i_in_kernel < TOTAL_KERN_LEN; i_in_kernel++){
//      printf("%f, %f\n", h_kernel[i_in_kernel].x, h_kernel[i_in_kernel].y);
//      h_kernel[i_in_kernel].x = 0.08544921875;
//      h_kernel[i_in_kernel].y = 0.0852050855755805969238; //0.0852050706744194030762 or 0.0852050855755805969238
//    }

 //   e = cudaMemcpy(d_kernel, h_kernel, KERNLEN*sizeof(float2)* NKERN, cudaMemcpyHostToDevice);
  ...
  void fdas_cuda_basic(fdas_cufftplan *fftplans, fdas_gpuarrays *gpuarrays, cmd_args *cmdargs, fdas_params *params, fdas_gpuarrays_float *gpuarrays_float)
  {
    ...
    */
    ...
    //complex block fft
      ...
      call_kernel_cuda_ffdotpow_concat_2d(pwblocks, pwthreads, gpuarrays->d_ffdot_cpx, gpuarrays->d_ffdot_pwr, params->sigblock, params->offset, params->nblocks, params->extlen, params->siglen);
      //call_kernel_cuda_ffdotpow_concat_2d_temp(pwblocks, pwthreads, gpuarrays->d_ffdot_cpx, temp_pwrs, params->sigblock, params->offset, params->nblocks, params->extlen, params->siglen);
      cudaDeviceSynchronize();
      //TIMEauto stop = high_resolution_clock::now();
      
      //TIMEauto duration = duration_cast<microseconds>(stop - start);
      //printf("\nTIME ON FFT STEP:\n");
      //TIMEcout << duration.count() << endl;
    //TIME}
  }
/*
#ifndef NOCUST
