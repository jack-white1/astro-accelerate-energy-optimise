...
namespace astroaccelerate {
  ...
  __global__ void cuda_convolve_reg_1d_halftemps(__nv_bfloat162* d_kernel, __nv_bfloat162* d_signal, __nv_bfloat162* d_ffdot_plane,int sig_tot_convlen, float scale)
  {
    ...
    __nv_bfloat16 bfscale = (__nv_bfloat16)scale;
...
#pragma unroll
      for (int i = 0; i < nTemplates; i++){
        ...
        __nv_bfloat162 tempvar = __hmul2(local_data, tempkern[i]);
        __nv_bfloat162 flippedvec = __lowhigh2highlow(tempkern[i]);
        __nv_bfloat162 tempvar2 = __hmul2(local_data, flippedvec);
        __nv_bfloat162 bfscale2 = __bfloat162bfloat162(bfscale);

        d_ffdot_plane[i*sig_tot_convlen + j + tidx ] = __hmul2(__halves2bfloat162(__hsub(tempvar.x,tempvar.y),__hadd(tempvar2.x,tempvar2.y)),bfscale2);
        d_ffdot_plane[(NKERN - 1 - i)*sig_tot_convlen + j + tidx ] = __hmul2(__halves2bfloat162(__hadd(tempvar.x, tempvar.y),__hsub(tempvar2.y,tempvar2.x)),bfscale2);
      ...
      __nv_bfloat162 tempvar3 = __hmul2(local_data,tempz);
      d_ffdot_plane[nTemplates*sig_tot_convlen + j + tidx ].x = __hmul(__hsub(tempvar3.x,tempvar3.y),bfscale);
      d_ffdot_plane[nTemplates*sig_tot_convlen + j + tidx ].y = __hmul(__hadd(__hmul(local_data.x,tempz.y),__hmul(local_data.y,tempz.x)),bfscale);
    }
  }
  ...
  //   //printf("tidy:%d,sig_totlen:%d,sigblock:%d,tidx:%d,total_blocks:%d\n",tidy,sig_totlen,sigblock,tidx,total_blocks);

  //   for (int i = 0; i <total_blocks; i++){
  //     if (tidx*2 < sigblock){
  //       __nv_bfloat162  local_data_0 =  __ldg(&d_ffdot_plane_cpx[(tidy * sig_tot_convlen) + i*KERNLEN  + 2*tidx + kern_offset ]) ;
  //       __nv_bfloat162  local_data_1 =  __ldg(&d_ffdot_plane_cpx[(tidy * sig_tot_convlen) + i*KERNLEN  + 2*tidx + 1 + kern_offset ]) ;

  //       __nv_bfloat162 squared_0 = __hmul2(local_data_0, local_data_0);
  //       __nv_bfloat162 squared_1 = __hmul2(local_data_1, local_data_1);

  //       //__nv_bfloat16  power_0 = __hadd(squared_0.x, squared_0.y);
  //       //__nv_bfloat16  power_1 = __hadd(squared_1.x, squared_1.y);
  //       // *(d_ffdot_plane + tidy*sig_totlen + i*sigblock + 2*tidx) = power_0;
  //       // *(d_ffdot_plane + tidy*sig_totlen + i*sigblock + 2*tidx + 1) = power_1;
        
  //       __nv_bfloat162 powers;
  //       powers.x = __hadd(squared_0.x, squared_0.y);
  //       powers.y = __hadd(squared_1.x, squared_1.y);

  //       *(d_ffdot_plane + tidy*sig_totlen + i*sigblock + 2*tidx) = powers.x;
  //       *(d_ffdot_plane + tidy*sig_totlen + i*sigblock + 2*tidx + 1) = powers.y;
  //       printf("%p,%p\n", d_ffdot_plane + tidy*sig_totlen + i*sigblock + 2*tidx, d_ffdot_plane + tidy*sig_totlen + i*sigblock + 2*tidx + 1);
  //       *((__nv_bfloat162 *)(d_ffdot_plane + tidy*sig_totlen + i*sigblock + 2*tidx)) = powers;
  //       //*((__nv_bfloat162*)(d_ffdot_plane + tidy*sig_totlen + i*sigblock + 2*tidx)) = __halves2bfloat162(power_0, power_1);
  ...
  /* calculates complex signal power and places result in float f-fdot array */
  {
    ...
    if (tidx*2 < sigblock){
      for (int i = 0; i <total_blocks; i++){
        ...
        //CUDA MEMORY ERRORS
        d_ffdot_plane[tidy*sig_totlen + i*sigblock + 2*tidx] = powers;
  ...
  void call_kernel_cuda_ffdotpow_concat_2d_temp(const dim3 &blocks, const dim3 &threads, __nv_bfloat162 *const d_ffdot_plane_cpx, __nv_bfloat162 *const d_ffdot_plane, const int &sigblock, const int &kern_offset, const int &total_blocks, const int &sig_tot_convlen, const int &sig_totlen) {
    dim3 dimBlock(32,1);
    cuda_ffdotpow_concat_2d_temp<<<blocks, dimBlock>>>(d_ffdot_plane_cpx, d_ffdot_plane, sigblock, kern_offset, total_blocks, sig_tot_convlen, sig_totlen);
  }


  __global__ void cuda_ffdotpow_concat_2d(__nv_bfloat162* d_ffdot_plane_cpx, __nv_bfloat16* d_ffdot_plane, int sigblock, int kern_offset, int total_blocks, int sig_tot_convlen, int sig_totlen)
  /* WORKINGCopies useful points from convolved complex f-fdot array (discarding contaminated ends) */
  /* calculates complex signal power and places result in float f-fdot array */
  {
    int tx = threadIdx.x;
    int ty = threadIdx.y;
  ...
  void call_kernel_cuda_ffdotpow_concat_2d(const dim3 &blocks, const dim3 &threads, __nv_bfloat162 *const d_ffdot_plane_cpx, __nv_bfloat16 *const d_ffdot_plane, const int &sigblock, const int &kern_offset, const int &total_blocks, const int &sig_tot_convlen, const int &sig_totlen) {
    //WORKING
    dim3 dimBlock(32,1);
    cuda_ffdotpow_concat_2d<<<blocks, dimBlock>>>(d_ffdot_plane_cpx, d_ffdot_plane, sigblock, kern_offset, total_blocks, sig_tot_convlen, sig_totlen);
  }
  ...
  //       __nv_bfloat16  power_0 = __hadd(squared_0.x, squared_0.y);
  //       __nv_bfloat16  power_1 = __hadd(squared_1.x, squared_1.y);
  //       //*((__nv_bfloat162*) &(d_ffdot_plane[(tidy * sig_totlen) + i*sigblock + tidx])) = __halves2bfloat162(power_0,power_1);
  //       //printf("local_data_0.x = %f, local_data_0.y=%f, power_0 = %f\n", (float)local_data_0.x,(float)local_data_0.y, (float) power_0);
  //       //*((__nv_bfloat162*)d_ffdot_plane[ (tidy * sig_totlen) + i*sigblock + tidx]) = __halves2bfloat162( power_0, power_1);
  //       //*(((__nv_bfloat162*)d_ffdot_plane)[ (tidy * sig_totlen) + i*sigblock + 2*tidx]) = __halves2bfloat162( power_0, power_1);
  //       //*((__nv_bfloat162*) &(d_ffdot_plane[(tidy * sig_totlen) + i*sigblock + 2*tidx])) = __halves2bfloat162( power_0, power_1);
  //       //d_ffdot_plane[1+(2*((tidy * sig_totlen) + i*sigblock + tidx))] = (__nv_bfloat16) 2.0*power_0;
  //       //d_ffdot_plane[ (tidy * sig_totlen) + i*sigblock + tidx] = power_0;
  //       //d_ffdot_plane[ (tidy * sig_totlen) + i*sigblock + tidx + 1] = power_1;
  //       //(__nv_bfloat162*) (d_ffdot_plane + (tidy * sig_totlen) + i*sigblock + tidx) = power_out;
  //     }
  //   }
  // }
  ...
  // /* calculates complex signal power and places result in float f-fdot array */
  // {
  //   //THIS VERSION WORKS AND OUTPUTS BFLOAT16S INTO A SINGLE PRECISION ARRAY FORMAT
  //   int tx = threadIdx.x;
  //   int ty = threadIdx.y;
  //   int bx = blockIdx.x;
  //   int by = blockIdx.y;
  //   int tidx = bx* blockDim.x + tx;
  //   int tidy = by* blockDim.y + ty;
  //   printf("tidy:%d,sig_totlen:%d,sigblock:%d,tidx:%d\n",tidy,sig_totlen,sigblock,tidx);

  //  for (int i = 0; i < total_blocks; ++i){
  //     if (tidx < sigblock){
  //       __nv_bfloat162  local_data =  __ldg(&d_ffdot_plane_cpx[(tidy * sig_tot_convlen) + i*KERNLEN  + tidx + kern_offset ]) ;
  //       //single compatible version
  //       d_ffdot_plane[1+(2*((tidy * sig_totlen) + i*sigblock + tidx))] = (__nv_bfloat16) 2.0*(local_data.x*local_data.x + local_data.y*local_data.y);
  //       //bfloat native version
  //       //d_ffdot_plane[(tidy * sig_totlen) + i*sigblock + tidx] = (__nv_bfloat16) 2.0*(local_data.x*local_data.x + local_data.y*local_data.y);
  //     }
  //   }
  // }
...
void call_kernel_set_float_array(__nv_bfloat16 *d_output, float *d_input, size_t data_length_bytes){
  int N_threads = 128;
...
void call_kernel_cast_float_to_bfloat16_down(__nv_bfloat16 *d_output, float *d_input, size_t data_length_bytes){
  ...
  //printf("Casting float to bfloat16: N_floats: %llu, N_blocks: %llu, true_N_floats: %lf, true_N_blocks: %lf\n", N_floats, N_blocks, true_N_floats, true_N_blocks);
  cast_float_to_bfloat16_down<<<N_blocks, N_threads>>>(d_output, d_input, N_floats);
}

__global__ void cast_float_to_bfloat16_down(__nv_bfloat16 *d_output, float *d_input, unsigned long long N_floats){
    unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N_floats){
      d_output[idx] = __float2bfloat16_rd(d_input[idx]);
    }
}
...
void call_kernel_cast_float_to_bfloat16_up(__nv_bfloat16 *d_output, float *d_input, size_t data_length_bytes){
  ...
  //printf("Casting float to bfloat16: N_floats: %llu, N_blocks: %llu, true_N_floats: %lf, true_N_blocks: %lf\n", N_floats, N_blocks, true_N_floats, true_N_blocks);
  cast_float_to_bfloat16_up<<<N_blocks, N_threads>>>(d_output, d_input, N_floats);
}

__global__ void cast_float_to_bfloat16_up(__nv_bfloat16 *d_output, float *d_input, unsigned long long N_floats){
    unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N_floats){
      d_output[idx] = __float2bfloat16_ru(d_input[idx]);
    }
}
...
void call_kernel_cast_float2_to_bfloat162_down(__nv_bfloat162 *d_output, float2 *d_input, size_t data_length_bytes){
  ...
  unsigned long long N_float2s = data_length_bytes/sizeof(float2);
  unsigned long long N_blocks = N_float2s/N_threads;
  cast_float2_to_bfloat162_down<<<N_blocks, N_threads>>>(d_output, d_input, N_float2s);
}

__global__ void cast_float2_to_bfloat162_down(__nv_bfloat162 *d_output, float2 *d_input, unsigned long long N_floats){
    unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N_floats){
      d_output[idx].x = __float2bfloat16_rd(d_input[idx].x);
      d_output[idx].y = __float2bfloat16_rd(d_input[idx].y);
    }
}
...
void call_kernel_cast_float2_to_bfloat162_up(__nv_bfloat162 *d_output, float2 *d_input, size_t data_length_bytes){
  ...
  unsigned long long N_float2s = data_length_bytes/sizeof(float2);
  unsigned long long N_blocks = N_float2s/N_threads;
  cast_float2_to_bfloat162_up<<<N_blocks, N_threads>>>(d_output, d_input, N_float2s);
}

__global__ void cast_float2_to_bfloat162_up(__nv_bfloat162 *d_output, float2 *d_input, unsigned long long N_floats){
    unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N_floats){
      d_output[idx].x = __float2bfloat16_ru(d_input[idx].x);
      d_output[idx].y = __float2bfloat16_ru(d_input[idx].y);
    }
}
