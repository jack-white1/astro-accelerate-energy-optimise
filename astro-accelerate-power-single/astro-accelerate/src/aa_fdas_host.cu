...
//      printf("%f, %f\n", h_kernel[i_in_kernel].x, h_kernel[i_in_kernel].y);
    ...
    //  h_kernel[i_in_kernel].y = 0.0852050706744194030762; //0.0852050706744194030762 or 0.0852050855755805969238
    //}
  ...
  void fdas_cuda_basic(fdas_cufftplan *fftplans, fdas_gpuarrays *gpuarrays, cmd_args *cmdargs, fdas_params *params)
  {
    ...
    */
    ...
    if (cmdargs->norm){
      ...
      //FILE *fp0;
      //printf("\n0_fft_before_dered.txt\n");
      //fp0 = fopen("0_fft_before_dered.txt", "w");
      //for (long int inSigCount = 0; inSigCount < params->rfftlen; inSigCount++){
      //  fprintf(fp0,"%f,%f\n",fftsig[inSigCount].x,fftsig[inSigCount].y);
      //}
	...
	LOG(log_level::error, "Could not cudaMemcpy in aa_fdas_host.cu (" + std::string(cudaGetErrorString(e)) + ")");
      ...
      printf("\n\n\nNUMAMPS AA: %ld\n\n\n",params->rfftlen);
      //deredden_presto_verbatim(fftsig, params->rfftlen);
    ...
    //FILE *fp1;
    //printf("\n1_fft_after_dered.txt\n");
    //fp1 = fopen("1_fft_after_dered.txt", "w");
    //for (long int inSigCount = 0; inSigCount < params->rfftlen; inSigCount++){
    //  fprintf(fp1,"%f,%f\n",fftsig[inSigCount].x,fftsig[inSigCount].y);
    //}
    ...
    //for (int ii = 0; ii < numRepeats; ii++){    
     ...
     /* if (cmdargs->norm){
        ...
        FILE *fp2;
        printf("\n2_extsig_after_overlapcopy.txt\n");
        fp2 = fopen("2_extsig_after_overlapcopy.txt", "w");
        for (long int inSigCount = 0; inSigCount < params->extlen; inSigCount++){
          fprintf(fp2,"%f,%f\n",extsig[inSigCount].x,extsig[inSigCount].y);
        }
      ...
      FILE *fp3;
      printf("\n3_extsig_after_norm.txt\n");
      fp3 = fopen("3_extsig_after_norm.txt", "w");
      for (long int inSigCount = 0; inSigCount < params->extlen; inSigCount++){
        fprintf(fp3,"%f,%f\n",extsig[inSigCount].x,extsig[inSigCount].y);
      }
...
/** \brief Write ffdot output data to disk. */
  void fdas_write_ffdot_cpx(fdas_gpuarrays *gpuarrays, cmd_args *cmdargs, fdas_params *params, float dm_low, int dm_count, float dm_step ) {
    int ibin=1;
    if (cmdargs->inbin)
      ibin=2;
    // Download, threshold and write ffdot data to file
    ...
    printf("\n\nWrite data for signal with %d samples\nf-fdot size=%u\n",params->nsamps, params->ffdotlen);
    float2 *h_ffdot_cpx = (float2*)malloc(params->ffdotlen* sizeof(float2));
    //download data
    cudaError_t e = cudaMemcpy(h_ffdot_cpx, gpuarrays->d_ffdot_cpx, params->ffdotlen*sizeof(float2), cudaMemcpyDeviceToHost);
    ...
    //filename needs to be acc_dm_%f, dm_low[i] + ((float)dm_count)*dm_step[i]
    //sprintf(pfname, "%s/out_inbin%d_%s",dirname,ibin,infilename);
    sprintf(pfname, "%splanecpx.dat",g_output_dat_filepath.c_str());//, dm_low + ((float)dm_count)*dm_step);
    printf("\nwriting results to file %s\n",pfname);
    if ((fp_c=fopen(pfname, "w")) == NULL) {
      fprintf(stderr, "Error opening %s file for writing: %s\n",pfname, strerror(errno));
      exit(1);
    ...
    for(long long int a = 0; a < NKERN; a++) {
      ...
      //printf("acc: %lf\n", acc);
      for(long long int j = 0; j < ibin*params->siglen; j++){
        pow.x =  h_ffdot_cpx[a * ibin*params->siglen + j].x;
        pow.y =  h_ffdot_cpx[a * ibin*params->siglen + j].y; //(h_ffdotpwr[a * params->siglen + j]-mean)/stddev;
        fprintf(fp_c, "%.3f\t%.3f\n", pow.x, pow.y);
      }
    }
    //fprintf(fp_c, "%.2lf\t%.3lf\t%lld\t%.3lf\t%.3f\t%.3f\n", temp_acc, temp_acc1, temp_j , temp_jfreq, temp_pow, temp_sigma);
