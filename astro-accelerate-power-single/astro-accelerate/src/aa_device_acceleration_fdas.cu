    ...
    */
	...
	for (int i = 0; i < range; i++) {
	  ...
	  for (int dm_count = 0; dm_count < ndms[i] - 1; ++dm_count) {
	    ...
	    //for (int signal_point = 0; signal_point < processed; signal_point++){
	    //	printf("before rand output_buffer[%d][%d][%d] = %f\n", i, dm_count, signal_point, output_buffer[i][dm_count][signal_point] );
	    //}

	    //for(int f=0; f<processed; f++) output_buffer[i][dm_count][f]=rand() / (float)RAND_MAX;
	    //for(int f=0; f<processed; f++) output_buffer[i][dm_count][f]=(float)1.0;

	    //for (int signal_point = 0; signal_point < processed; signal_point++){
	    //	printf("after rand output_buffer[%d][%d][%d] = %f\n", i, dm_count, signal_point, output_buffer[i][dm_count][signal_point] );
	    //}

	    /*printf("\nPRINTING input signal to inputsignal.txt  in format: float\n");

    	FILE *fp;
    	fp = fopen("inputsignal.txt", "w");

	    for (long int inSigCount = 0; inSigCount < processed; inSigCount++){
	    	fprintf(fp,"%f\n",output_buffer[i][dm_count][inSigCount]);
	    }

	    double inputMean = 0;
	    for (long int inSigCount = 0; inSigCount < processed; inSigCount++){
	    	inputMean = inputMean + (double)output_buffer[i][dm_count][inSigCount];
	    }
	    inputMean = inputMean / processed;

	    double sumSqDiff = 0;
	    for (long int inSigCount = 0; inSigCount < processed; inSigCount++){
	    	sumSqDiff = sumSqDiff + (double) (output_buffer[i][dm_count][inSigCount] - inputMean)*(output_buffer[i][dm_count][inSigCount] - inputMean);
	    }

	    double inStDev = sqrt(sumSqDiff / (processed - 1));
	    for (long int inSigCount = 0; inSigCount < processed; inSigCount++){
	    	output_buffer[i][dm_count][inSigCount] = (float) 10000*(output_buffer[i][dm_count][inSigCount] - inputMean)/inStDev;
	    }

	    printf("\nPRINTING normalised input signal to normalisedinputsignal.txt  in format: float\n");
	    printf("inputMean = %f, sumSqDiff = %f, inStDev = %f, processed = %d\n", inputMean, sumSqDiff,inStDev, processed);

    	FILE *fp2;
    	fp2 = fopen("normalisedinputsignal.txt", "w");
